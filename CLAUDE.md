...
- **GitHub Actions workflow YAML**: A `run:` step whose command contains a `: ` (colon-space) — common in f-strings like `print(f'PyGhidra version: {pyghidra.__version__}')` — is parsed by YAML as a key/value separator and fails validation (`error in your yaml syntax on line N`). Put such commands under a `run: |` block scalar (or quote the whole value), never an inline plain scalar. After editing any workflow `.yml`, parse-validate it before committing: `uv run --with pyyaml python -c "import yaml,sys; yaml.safe_load(open('.github/workflows/<file>.yml'))"` (bare `python3` here has no PyYAML).
