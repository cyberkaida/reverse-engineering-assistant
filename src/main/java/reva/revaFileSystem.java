...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = revaFileSystem.MyFileSystemFactory.class)
public class revaFileSystem implements GFileSystem {
	 ...
	 */
	public revaFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + revaFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<revaFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public revaFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			revaFileSystem fs = new revaFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
