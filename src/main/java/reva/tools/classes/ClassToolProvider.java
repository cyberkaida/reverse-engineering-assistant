...
public class ClassToolProvider extends AbstractToolProvider {
    ...
    public void registerTools() throws McpError {
        ...
        registerAssociateFunctionWithClassTool();
        ...
        registerTool(tool, (exchange, args) -> {
            try {
                ...
                while (classIterator.hasNext() && classes.size() < maxCount) {
                    ...
                    Map<String, Object> classInfo = createBasicClassInfo(ghidraClass, functionCount);
    ...
    private void registerListClassMethodsTool() throws McpError {
        ...
        registerTool(tool, (exchange, args) -> {
            try {
                ...
                List<Map<String, Object>> methods = getClassMethods(classNamespace, program);
    ...
    private void registerCreateClassTool() throws McpError {
        ...
        McpSchema.Tool tool = new McpSchema.Tool(
            "create-class",
            "Create a new class for reconstructed class information",
            createSchema(properties, required)
        );
     ...
     * @throws McpError if there's an error registering the tool
     */
    private void registerAssociateFunctionWithClassTool() throws McpError {
        Map<String, Object> properties = new HashMap<>();
        properties.put("programPath", SchemaUtil.stringProperty("Path in the Ghidra Project to the program"));
        ...
        properties.put("className", SchemaUtil.stringProperty("Name of the class to associate the function with"));
        ...
        List<String> required = List.of("programPath", "functionName", "className");

        McpSchema.Tool tool = new McpSchema.Tool(
            "associate-function-with-class",
            "Associate a function with a class by moving it to the class namespace. This is useful for organizing reconstructed methods under their appropriate classes.",
            createSchema(properties, required)
        );
        ...
        registerTool(tool, (exchange, args) -> {
            try {
                ...
                if (parentNamespace != null && classNamespace.equals(functionSymbol.getParentNamespace())) {
                    return createErrorResult("Function '" + function.getName() + "' is already associated with class '" + className + "'");
                }
                ...
                Msg.error(this, "Error associating function with class", e);
                return createErrorResult("Error associating function with class: " + e.getMessage());
            }
        });
    }
     ...
     */
    private Map<String, Object> createBasicClassInfo(Namespace classNamespace, int functionCount) {
    ...
    private Map<String, Object> createDetailedClassInfo(Namespace classNamespace, Program program) {
        ...
        List<Map<String, Object>> methods = getClassMethods(classNamespace, program);
        Map<String, Object> info = createBasicClassInfo(classNamespace, methods.size());
     ...
     */
    private List<Map<String, Object>> getClassMethods(Namespace classNamespace, Program program) {
