...
**Structural diff + artifact classification.** `diff-function` calls `createDiff(before, after, 2, true, includeArtifactHunks)` — the `true` selects the LCS-aligned structural path (a mid-body insertion reports as one ADDED line instead of cascading every following line) with address-shift masking in the equality test. Each surviving hunk is then classified `code` (a real change, shown in full) or a decompiler artifact: `var-renumber` (the decompiler renumbered auto-vars like `uVar7`↔`uVar8` between builds) or `string-label` (only a string-label's embedded content — e.g. a per-build path — differs). Artifact hunks are **collapsed by default** (tagged + counted under `suppressedHunks`, content omitted); pass `includeArtifactHunks: true` to expand them. A hunk containing *any* genuine line stays `code`. The summary counts only `code` hunks. Address masking is equality-only (snippets show original identifiers); `var-renumber`/`string-label` use classify-and-collapse (not equality masking) so a real variable swap or format-string change is never silently hidden. The flag-off path (`get-decompilation`) keeps the index-based diff, byte-identical.
