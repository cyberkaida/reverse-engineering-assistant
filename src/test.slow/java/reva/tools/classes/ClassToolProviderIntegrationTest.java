...
public class ClassToolProviderIntegrationTest extends RevaIntegrationTestBase {
    ...
    /**
     * Test that reconstruct-classes-from-rtti tool with runAnalysis parameter works.
     */
    @Test
    public void testRttiReconstructionWithRunAnalysis() throws Exception {
        withMcpClient(createMcpTransport(), client -> {
            try {
                ...
                // Should mention analysis or provide some feedback
                assertTrue("Should mention analysis or provide useful feedback. Actual response: " + resultText,
                          resultText.contains("analysis") || 
                          resultText.contains("RTTI") ||
                          resultText.contains("class") ||
                          resultText.length() > 50); // Should have substantial content
    ...
    /**
     * Test that get-class-info tool works properly (existing functionality).
     */
    @Test
    public void testGetClassInfo() throws Exception {
        withMcpClient(createMcpTransport(), client -> {
            try {
                ...
                assertFalse("Tool call should succeed (even if class not found)", result.isError());
                assertNotNull("Should have content", result.content());
                
                String resultText = getResultText(result);
                
                // Should either find the class or indicate it wasn't found
                assertTrue("Should provide meaningful response about class. Actual response: " + resultText,
                          resultText.contains("TestClass") || 
                          resultText.contains("not found") ||
                          resultText.contains("No class") ||
                          resultText.contains("class"));
